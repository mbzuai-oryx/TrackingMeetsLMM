...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(self, llama_ckpt_dir, llama_tokenizer,
        ...
        self.traj_proj = nn.Linear(1024, v_embed_dim)
        self.ego_proj = nn.Linear(1024, v_embed_dim)
    ...
    def get_trainable_params(self, phase='finetune'):
        ...
        if phase == 'finetune':
            for name, para in self.named_parameters():
                if 'traj' in name or 'ego' in name:
            ...
            train_param_name = ['traj', 'ego']#, 'gate', 'clip_proj', 'clip_proj_norm', 'visual_query', 'visual_blocks', 'visual_proj', 'visual_proj_norm', 'adapter_query']
            for name, para in self.named_parameters():
    ...
    def forward_visual(self, inputs):
        clip_feats = []
        imgs = inputs['Image'][0]
        traj = inputs['traj'][0]
        ego = inputs['ego'][0]
        ...
        ego_feats = F.normalize(self.traj_encoder({'ego' : ego})['ego'], dim=-1)
        ego_feats = self.ego_proj(ego_feats.unsqueeze(1).float())

        traj_feats = F.normalize(self.traj_encoder({'traj' : traj})['traj'], dim=-1)
        traj_feats = self.traj_proj(traj_feats.unsqueeze(1).float())
    ...
    def forward(self, tokens, labels, imgs, trajs=None, egos=None):

        visual_query = self.forward_visual({'Image': [imgs, 1], 'traj': [trajs, 1], 'ego': [egos, 1]})
        ...
        if labels.sum() == 0:
            ...
            if not math.isfinite(c_loss):
                print('output and labels', output, labels)
                print('input', tokens)
                print('vis que', visual_query)
